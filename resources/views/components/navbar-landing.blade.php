    ...
    <div class="container">
        <a class="navbar-brand" href="{{ route('home') }}">
            @if (Route::has('login'))
                ...
                @else
                    <img src="{{ asset('img-more/avatar-1.png') }}" class="rounded-circle mr-1" height="35" />
                <span class="font-weight-bold fs-3">Halo, apa kabar hari ini? 👋</span>
                @endauth
            @endif
        </a>
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav ml-auto text-center fs-4">
            ...
            @if (Route::has('login'))
                <ul class="navbar-nav ml-auto text-center">
                    ...
                    @else
                    <li class="nav-item my-1 mx-1">
                        <a class="nav-link btn btn-light btn-sm px-3 fs-4 rounded-double"
                            href="{{ route('login') }}">Masuk</a>
                    </li>
                    <li class="nav-item my-1 mx-1">
                        <a class="nav-link btn btn-primary text-white btn-sm px-3 fs-4 rounded-double" href="{{ route('register') }}">Daftar</a>
                    </li>
                    @endauth
                </ul>
            @endif
